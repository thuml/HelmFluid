...
class HelmBlock(nn.Module):
    def __init__(self, input_dim, out_dim, correlation_dim=None, encoder_dim=None, groups=8, patch_size=2):
        ...
        self.vorticity_weight = nn.Parameter(torch.tensor([0.05]))
    ...
    def forward(self, prev, next, texture, helm=None):
        ...
        print(time()-t, 'forward')
        t = time()
        print('HelmBlock Finished\n')
...
class Model(nn.Module):
    def __init__(self, args, bilinear=True):
        ...
        self.process1 = HelmBlock(width, width, correlation_dim, groups=self.groups)
        self.process2 = HelmBlock(width * 2, width * 2, correlation_dim, groups=self.groups)
        self.process3 = HelmBlock(width * 4, width * 4 // factor, correlation_dim, groups=self.groups)
