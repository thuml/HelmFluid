...
class HelmBlock(nn.Module):
    def __init__(self, input_dim, out_dim, correlation_dim=None, encoder_dim=None, groups=8, patch_size=4):
        ...
        self.vorticity_weight = nn.Parameter(torch.tensor([0.01]))
        self.phi_weight = nn.Parameter(torch.tensor([0.01]))
...
class Model(nn.Module):
    def __init__(self, args, bilinear=True):
        ...
        self.process3 = HelmBlock(width * 4, width * 4 // factor, 512, groups=self.groups) # original 1024
        # projectors
