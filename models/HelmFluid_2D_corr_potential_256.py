...
def aggregate_to_velocity(phi, vorticity, corr_size):
    vel_phi = calc_phi_velocity(phi, corr_size)
    vel_vorticity = calc_vort_velocity(vorticity, corr_size)

    return vel_phi+vel_vorticity, vel_phi, vel_vorticity
...
class HelmBlock(nn.Module):
    def __init__(self, input_dim, out_dim, correlation_dim=None, encoder_dim=None, groups=8, patch_size=4):
        ...
        self.vorticity_decoder = nn.Sequential(
            DoubleConv(neighbours, encoder_dim // groups, up=True),
    ...
    def forward(self, prev, next, texture, helm=None):
        shape = texture.shape[-2:]
        B, C, H, W = prev.shape

        next = next.reshape([B * self.groups, C // self.groups, H, W])
        prev = prev.reshape([B * self.groups, C // self.groups, H, W])
        prev_features = self.encoder(prev)
        next_features = self.encoder(next)
        correlation = FunctionCorrelation(prev_features, next_features)
...
class Model(nn.Module):
    def __init__(self, args, bilinear=True):
        ...
        if self.dataset_name == 'sea_temperature':
            correlation_dim = 256
        else:
            correlation_dim = None

        self.process1 = HelmBlock(width, width, 256, groups=self.groups) # original 1024
        self.process2 = HelmBlock(width * 2, width * 2, 512, groups=self.groups) # original 1024
        self.process3 = HelmBlock(width * 4, width * 4 // factor, 1024, groups=self.groups) # original 1024
        # projectors
        self.padding = padding
        self.texture_fc0 = nn.Linear(in_channels + 2, width)
        self.feature_fc0 = nn.Linear(in_channels - 1, width)
    ...
    def forward(self, x, freeze=False):
        ...
        if self.dataset_name == 'z500_era5':
            ...
            prev = torch.nn.functional.pad(prev, [0,0,8,8], mode='reflect')
            prev = torch.nn.functional.pad(prev, [8,8,0,0], mode='circular')
            prev = prev.permute(0, 2, 3, 1).contiguous()
            next = next.permute(0, 3, 1, 2)
            next = torch.nn.functional.pad(next, [0,0,8,8], mode='reflect')
            next = torch.nn.functional.pad(next, [8,8,0,0], mode='circular')
            next = next.permute(0, 2, 3, 1).contiguous()
        elif self.dataset_name == 'NS':
            prev = prev.permute(0, 3, 1, 2)
            prev = torch.nn.functional.pad(prev, [32,32,32,32], mode='circular')
            prev = prev.permute(0, 2, 3, 1).contiguous()
            next = next.permute(0, 3, 1, 2)
            next = torch.nn.functional.pad(next, [32,32,32,32], mode='circular')
            next = next.permute(0, 2, 3, 1).contiguous()
        elif self.dataset_name == 'sea_temperature':
            prev = prev.permute(0, 3, 1, 2)
            prev = torch.nn.functional.pad(prev, [16, 16, 16, 16], mode='reflect')
            prev = prev.permute(0, 2, 3, 1).contiguous()
            next = next.permute(0, 3, 1, 2)
            next = torch.nn.functional.pad(next, [16, 16, 16, 16], mode='reflect')
            next = next.permute(0, 2, 3, 1).contiguous()

        prev = self.feature_fc0(prev)
        next = self.feature_fc0(next)
        prev = prev.permute(0, 3, 1, 2)
        next = next.permute(0, 3, 1, 2)
        ...
        helm = None
        # vel_vort = None
        x3, helm, vel, _, _ = self.process3(prev_x3, next_x3, x3, helm)
        x2, helm, vel, _, _ = self.process2(prev_x2, next_x2, x2, helm)
        x1, helm, vel, _, _ = self.process1(prev_x1, next_x1, x1, helm)
        ...
        if self.dataset_name == 'z500_era5':
            ...
            helm = helm.reshape([B, self.groups, 2, H+64, W+64])[:, 0, ..., 32:-32, 32:-32]
        else:
