...
class HelmBlock(nn.Module):
    ...
    def forward(self, prev, next, texture, helm=None):
        ...
        next = next.reshape([B * self.groups, C // self.groups, H, W]).contiguous()
        prev = prev.reshape([B * self.groups, C // self.groups, H, W]).contiguous()
...
class Model(nn.Module):
    def __init__(self, args, bilinear=True):
        ...
        # multiscale modules
        self.texture_inc = Down(width, width)
        ...
        self.outc = nn.Sequential(nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
        DoubleConv(width, width))

        # Multiscale Helmholtz Warping Block
        self.feature_inc = Down(width, width)
        ...
        self.process1 = HelmBlock(width, width, correlation_dim, groups=self.groups) # original 1024
        self.process2 = HelmBlock(width * 2, width * 2, correlation_dim, groups=self.groups) # original 1024
        self.process3 = HelmBlock(width * 4, width * 4 // factor, correlation_dim, groups=self.groups) # original 1024
        # projectors
    ...
    def forward(self, x, freeze=False):
        ...
        if self.dataset_name == 'z500_era5':
            ...
            prev = prev.permute(0, 3, 1, 2)
            prev = torch.nn.functional.pad(prev, [16,16,16,16], mode='circular')
            prev = prev.permute(0, 2, 3, 1).contiguous()
            next = next.permute(0, 3, 1, 2)
            next = torch.nn.functional.pad(next, [16,16,16,16], mode='circular')
            next = next.permute(0, 2, 3, 1).contiguous()
