...
class Model(nn.Module):
    ...
    def forward(self, x, mask, boundary):
        ...
        if 'boundary' in self.dataset_name:
            ...
            prev = torch.nn.functional.pad(prev, [0,0,32,32,32,32], mode='replicate')
            next = torch.nn.functional.pad(next, [0,0,32,32,32,32], mode='replicate')
            feature_mask = torch.nn.functional.pad(input_mask, [0,0,32,32,32,32], mode='constant')
            mask = torch.nn.functional.pad(mask * 1.0, [32,32,32,32], mode='constant') > 0
            boundary = torch.nn.functional.pad(boundary * 1.0, [32,32,32,32], mode='constant') > 0
            mask = mask.to(x.device)
            boundary = boundary.to(x.device)
