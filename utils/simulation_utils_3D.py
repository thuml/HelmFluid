...
def RK4(pos, u, dt):
    k1 = u
    k2 = sample_grid_batched(u, pos + 0.5 * dt * k1)
    k3 = sample_grid_batched(u, pos + 0.5 * dt * k2)
    k4 = sample_grid_batched(u, pos + dt * k3)
    return pos + dt * (1/6 * k1 + 1/3 * k2 + 1/3 * k3 + 1/6 * k4)
...
def index_take_3D(source, index_x, index_y, index_z):
    W, H, T, Channel = source.shape
    W_, H_, T_ = index_x.shape
    index_flattened_x = index_x.flatten()
    index_flattened_y = index_y.flatten()
    index_flattened_z = index_z.flatten()
    sampled = source[index_flattened_x, index_flattened_y, index_flattened_z].view((W_, H_, T_, Channel))
    return sampled

index_take_batched = vmap(index_take_3D)

# clipping used for MacCormack and BFECC
def MacCormack_clip(advected_quantity, quantity, u, x, dt, boundary):
    batch, W, H, T, _ = u.shape
    # prev_pos = RK3(x, u, -1. * dt) # [batch, W, H, 2]
    # prev_pos = RK1(x, u, -1. * dt) # [batch, W, H, 2]
    # prev_pos = RK4(x, u, -1. * dt) # [batch, W, H, 2]
    prev_pos = RK2(x, u, -1. * dt) # [batch, W, H, 2]
    ...
    pos_grid_z = torch.clamp(pos_grid[..., 2], 0, T-2)
    pos_grid_x_plus = pos_grid_x + 1
    pos_grid_y_plus = pos_grid_y + 1
    pos_grid_z_plus = pos_grid_z + 1
    BLF = index_take_batched(quantity, pos_grid_x, pos_grid_y, pos_grid_z)
    BRF = index_take_batched(quantity, pos_grid_x_plus, pos_grid_y, pos_grid_z)
    TRF = index_take_batched(quantity, pos_grid_x_plus, pos_grid_y_plus, pos_grid_z)
    TLF = index_take_batched(quantity, pos_grid_x, pos_grid_y_plus, pos_grid_z)


    BLB = index_take_batched(quantity, pos_grid_x, pos_grid_y, pos_grid_z_plus)
    BRB = index_take_batched(quantity, pos_grid_x_plus, pos_grid_y, pos_grid_z_plus)
    TRB = index_take_batched(quantity, pos_grid_x_plus, pos_grid_y_plus, pos_grid_z_plus)
    TLB = index_take_batched(quantity, pos_grid_x, pos_grid_y_plus, pos_grid_z_plus)
    stacked = torch.stack((BLF, BRF, TRF, TLF, BLB, BRB, TRB, TLB), dim = 0)
...
# data = [batch, X, Y, Z, n_channel]
# pos = [batch, X, Y, Z, 3]
def sample_grid_batched(data, pos):
    data_ = data.permute([0, 4, 3, 2, 1])# b, c, z, Y, X
    pos_ = pos.clone().permute([0, 3, 2, 1, 4])# b, z, Y, X, 3
    pos_ = (pos_ - 0.5) * 2
    # b, c, z, y, x
    F_sample_grid = F.grid_sample(data_, pos_, padding_mode = 'reflection', align_corners = False, mode = "bilinear")
    F_sample_grid = F_sample_grid.permute([0, 4, 3, 2, 1])# b, x, y, z, c
